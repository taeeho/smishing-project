  ...
  return (
    <div className="space-y-6">
      <header className="rounded-2xl border border-violet-100 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">최근 1년 동안 사용자들이 자주 겪은 스미싱 유형을 모아 보여줍니다.</p>
      </header>
      ...
      <section className="rounded-2xl border border-violet-100 bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          {AGE_GROUPS.map((age) => (
            <button key={age} type="button" onClick={() => setActiveAge(age)}
              className={[
      ...
      <section className="rounded-2xl border border-violet-100 bg-white p-5 shadow-sm">
        ...
        {!loading && !error && items.length === 0 && (
          <div className="mt-4 rounded-xl border border-violet-100 bg-violet-50/40 px-4 py-8 text-center text-sm text-slate-400">표시할 트렌드 데이터가 없습니다.</div>
        )}

        {!loading && !error && items.length > 0 && (
          <div className="mt-4 space-y-3">
            {items.map((item, idx) => (
              <div key={`${item.label}-${idx}`} className="flex items-center justify-between rounded-xl border border-violet-100 bg-violet-50/60 px-4 py-3">
